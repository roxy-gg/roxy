...
Un miembro es **Roxy con una especialidad**: su brief se *anade* al prompt base,
...
1. **El roster no entraba al prompt.** Solo se inyectaba el brief del miembro
   ...
   equivocada: un subagente es un hijo en blanco de *su propio* contexto, que
...
2. **El enrutado le daba el trabajo entero al mencionado.** Se tomaba la
   *primera* mencion en cualquier posicion, asi que "crea el comando, crea el PR
   ...
   frase es hablar *sobre* alguien, y el turno se queda con el host, que hace el
...
3. **El brief se leia como la tarea.** Pegado crudo, un bot con "revisas PRs y
   ...
   brief va enmarcado como *standing identity*, y lo que decide que hacer es el
   ultimo mensaje del canal.

4. **Gemini rechazaba el hand-off con 400.** En un relevo nadie escribe un
   mensaje nuevo, asi que el transcript *terminaba* en un turno de assistant:
...
- **One-line role** -> `role`. Etiqueta corta. Se muestra junto al nombre y en
  el roster que ven los otros bots. *Tambien va al prompt*, en la linea de
...
- Las skills son por *agente* y workspace, no por miembro: todos los bots ven
